...
public class SendEmailServiceImpl implements SendEmailService {
    ...
    public ResponseResult<Object> sendEmailCode(String email) {
        ...
        if (expire == null || expire < SystemConstant.CAPTCHA_CHECK_TTL) {
            ...
            if (count > 0) {
                MailUtil.send(email, "凌云博客重置密码", "小伙伴您好！您正在重置密码，请勿泄露您的验证码，您的验证码是：【" + captcha + "】，请尽快使用，有效时间为5分钟！", false);
            } else {
                MailUtil.send(email, "凌云博客注册", "小伙伴您好！欢迎注册凌云博客账号，您的验证码是：【" + captcha + "】，请尽快使用，有效时间为5分钟！", false);
            }
